...
def stock_vol_factor(df, m, lags_std, lags_pct):
    log_rtn = np.log(df).diff(1)
    def std(lag):
        std = log_rtn.rolling(lag).std()
        return std
    df_std = std(lags_std[0]) + std(lags_std[1]) + std(lags_std[2])
    def pct(lag):
        pct = df_std.rolling(m).mean().pct_change(lag)
        return pct
    df_factor = -1 * (pct(lags_pct[0]) + pct(lags_pct[1]) + pct(lags_pct[2]))
    return df_factor

def stock_vol(df, *, m=5, lags_std=[250, 300, 350], lags_pct=[240, 270, 300], positions=[0.5, 0.5, -0.5, -0.5]):
    df_factor = stock_vol_factor(df, m, lags_std, lags_pct)
    df_position = factor_to_position(df_factor, positions)
    df_price = df.copy()
    return df_price, df_position
...
def bond_value(df, ytm_origin, *, transform_matrix, m=5, lags=[250, 500, 750], positions=[0.5, 0.5, 0, -0.5, -0.5]):
...
# 3.YTM滚动10日平均
# 4. YTM对对应期限的斜率
# 5. 对斜率， 100 * （最近12天均值 - 之前50天），以及100天和150天
# 6. 对上述求均值。并称之为套索
# 7. 套索为正，买短卖长，套索为负，买长卖短
def bond_carry_ytm(df, df_rf, ytm_origin, transform_matrix, m1):
    transform_matrix = np.array(transform_matrix).T
    ytm_transform = ytm_origin.dot(transform_matrix)
    ytm_transform.columns = df.columns
    ytm_transform = pd.concat([df_rf, ytm_transform], axis=1)
    ytm_transform = ytm_transform.rolling(m1).mean().shift(1)
    ytm_transform.fillna(method="ffill", inplace=True)
    return ytm_transform.dropna()

def bond_carry_facotr(ytm_transform, m2, x, lags):
    def slope(x, y):
    ...
    def carry(lag):
        df = (df_slope.rolling(m2).mean() - df_slope.rolling(lag).mean()) * 100
        return df
    df_carry = (carry(lags[0]) + carry(lags[1]) + carry(lags[2])) / 3
    return df_carry
...
def bond_carry(df, df_rf, ytm_origin, *, transform_matrix, x=[7/365, 2, 4, 6, 17/2, 50/3], m1=10, m2=60, lags=[250, 500, 750], positions=[0.5, 0.5, 0, -0.5, -0.5]):
    ytm_transform = bond_carry_ytm(df, df_rf, ytm_origin, transform_matrix, m1)
    df_carry = bond_carry_facotr(ytm_transform, m2, x, lags)
